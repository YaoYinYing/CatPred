...
if __name__ == '__main__':
    # Dataset Load
    train = pd.read_csv('../data/CatPred-DB/data/kcat/kcat-random_trainval.csv')[:]
    test = pd.read_csv('../data/CatPred-DB/data/kcat/kcat-random_test.csv')[:]
    sequence_train = train['sequence']
    sequence_test = test['sequence']
    smiles_train = train['reactant_smiles']
    ...
    for i in range(10):
        print(os.getcwd)
    if not os.path.exists('../data/external/UniKP/datasets/catpred_kcat/kcat-random_trainval.pkl') or not os.path.exists('./external/UniKP/datasets/catpred_kcat/kcat-random_test.pkl'):
        ...
        with open('../data/external/UniKP/datasets/catpred_kcat/kcat-random_trainval.pkl', "wb") as f:
            pickle.dump(feature_train, f)
        with open('../data/external/UniKP/datasets/catpred_kcat/kcat-random_test.pkl', "wb") as f:
            pickle.dump(feature_test, f)
    else:
        with open('../data/external/UniKP/datasets/catpred_kcat/kcat-random_trainval.pkl', "rb") as f:
            feature_train = pickle.load(f)
        with open('../data/external/UniKP/datasets/catpred_kcat/kcat-random_test.pkl', "rb") as f:
